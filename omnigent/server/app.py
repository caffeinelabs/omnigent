...
class ServerInfoResponse(BaseModel):
    ...
    # SSHPiper gateway coordinates for the web UI's "Open in VS Code" (Remote-SSH)
    # button; all None when no SSHPiper gateway is configured.
    sshpiper_host: str | None = None
    sshpiper_port: int | None = None
    sshpiper_user: str | None = None
...
def create_app(
    ...
    # SSHPiper gateway config (fork feature): advertised on /v1/info so the web
    # UI can offer "Open in VS Code" (Remote-SSH); None hides it.
    ...
    async def info() -> ServerInfoResponse:
        ...
        # SSHPiper gateway coords for the "Open in VS Code" button (fork feature).
        _sshpiper = getattr(app.state, "sshpiper_config", None)
        return ServerInfoResponse.model_validate(
            {
                ...
                "sshpiper_host": _sshpiper.host if _sshpiper is not None else None,
                "sshpiper_port": _sshpiper.port if _sshpiper is not None else None,
                "sshpiper_user": _sshpiper.user if _sshpiper is not None else None,
