...
launch token expires or the host row is deleted (the session's host teardown) —
so it is revocable and executor agnostic. Responses are ``Cache-Control:
no-store`` so no intermediary retains the token. See
