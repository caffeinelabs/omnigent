...
Mounted under ``/v1`` so paths are ``/v1/connections/github/...``.
...
def create_connections_github_router(
    ...
    *,
    ...
    # The OAuth state is HMAC-signed with a subkey derived from the App's own
    # client secret (a required, high-entropy secret owned by this flow) via a
    # purpose-bound KDF, so the raw secret is never the signing key. State is
    # short-lived (``_STATE_TTL_S``), so a client-secret rotation only
    # invalidates in-flight connect attempts, never stored connections.
    state_key = _derive_state_signing_key(config.client_secret)
    ...
    def _sign_state(user_id: str, return_to: str) -> str:
        ...
        return jwt.encode(payload, state_key, algorithm=_STATE_ALG)

    def _verify_state(state: str) -> dict:
        return jwt.decode(state, state_key, algorithms=[_STATE_ALG])

    @router.get("/connections/github/status")
    ...
    async def _resolve_session_pulls(
        user_id: str, session_id: str, conv: Any
    ) -> dict[str, object]:
    ...
    @router.get("/connections/github/sessions/{session_id}/pull-requests")
