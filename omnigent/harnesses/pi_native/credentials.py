...
def _inline_family_pi_provider(
    ...
    for family_name in _inline_family_order(model):
        ...
        # tier map) alongside the selected model, so Pi's /model picker offers
        # exactly the deployment's verified set instead of just the launch
        # model. Duplicate ids collapse to their first occurrence. Each entry
        # is enriched via _gateway_pi_model_entry (real context-window /
        # max-output / input metadata) so large-context gateway models aren't
        # silently truncated to Pi's 128k default.
        ...
        for tier_model in family.models.values():
            ...
            entry_model = _gateway_pi_model_entry(
                tier_model,
                configured_context_window=family.context_window,
                configured_max_output_tokens=family.max_output_tokens,
            )
            # Deepseek (pi_model_is_reasoning) streams reasoning on every
            # surface, so keep its flag. Claude only surfaces a thinking channel
            # on the anthropic-messages surface; _gateway_pi_model_entry flags it
            # by id, so strip that flag on an openai-completions/responses
            # surface where it would be spurious.
            if (
                api != "anthropic-messages"
                and entry_model.get("reasoning")
                and not pi_model_is_reasoning(tier_model)
            ):
                del entry_model["reasoning"]
