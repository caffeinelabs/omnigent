...
``account_tokens.kind``, ``policies.type``, ``hosts.status``) are stored
as integer codes rather than their string names — smaller rows and a
tighter ``CHECK`` than a free ``VARCHAR``. The string names remain the
