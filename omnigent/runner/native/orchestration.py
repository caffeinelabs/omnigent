...
async def _auto_create_opencode_terminal(
    ...
    *,
    ...
    if gateway is None:
        # Fork fallback: a deployment-wide OpenAI-compatible gateway declared via
        # HARNESS_OPENCODE_GATEWAY_* (keyless in-cluster Bifrost), consulted only
        # when neither a bound provider nor a Databricks profile resolves.
