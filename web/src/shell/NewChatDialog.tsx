...
export function NewChatLandingScreen() {
  ...
  // Each is cloned side by side under the sandbox workspace root. An
  // in-progress draft (returning to the composer) wins; otherwise pre-fill the
  // repos the user launched their last session with, so a repeat session
  // doesn't start from an empty picker.
  const [selectedSandboxRepos, setSelectedSandboxRepos] = useState<SelectedSandboxRepo[]>(
    () => restoredDraft?.sandboxRepos ?? readLastSandboxRepos() ?? [],
  );
          ...
          body: JSON.stringify({
            ...
            ...(sandboxSelected
              ? {
                  ...
                  // `workspace`. On a `project_id` create an ABSENT workspace
                  // would be default-filled with the config's path workspace,
                  // which a managed create rejects — pin an explicit null
                  // instead (explicit values are never replaced by hints).
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      <span
                        className={`hidden max-w-40 truncate text-sm sm:block ${sandboxRepoChosen ? "text-foreground" : "text-muted-foreground"}`}
                      >
                        {sandboxRepoLabel}
                      </span>
                      <ChevronDownIcon className="size-3.5 shrink-0 opacity-60" />
                      ...
                      {sandboxRepoPickerConnected ? (
                        <>
                          ...
                          {sandboxReposTruncated && (
                            ...
                            >
                              Showing your most recently pushed repositories. Don't see one? Search
                              above, or paste its URL after picking none.
                            </p>
                          )}
                          {selectedSandboxRepos.length > 0 && (
                            <div
                        ...
                        <>
                          ...
                          {githubReposEnabled && sandboxReposErrored && (
                            <p
