...
describe("NewChatLandingScreen", () => {
  ...
  it("offers a GitHub multi-repo picker that sends the picked repos as workspaces", async () => {
    // enabled_connections has github + a connected /repos response → the
    // multi-select picker renders inside the repo chip; picked repos are sent
    // as the managed `workspaces` list, each with its chosen branch.
    ...
    // Filter by typing, then toggle the repo into the selection.
    ...
    // The repo appears in the selected list; pick its branch from the per-repo
    // <select> (its branches loaded from the /branches endpoint).
    const selectedRow = await screen.findByTestId("new-chat-landing-repo-selected-row");
    expect(selectedRow.textContent).toContain("octo/hello");
    const branchSelect = await screen.findByTestId("new-chat-landing-repo-branch-select");
    await waitFor(() =>
      expect(within(branchSelect as HTMLElement).getByRole("option", { name: "dev" })).toBeTruthy(),
    );
    ...
    await waitFor(() => {
      const postCall = authenticatedFetchMock.mock.calls.find(
    ...
    // The connected picker sends the composed `workspaces` list, not a single
    // free-text `workspace`.
    expect(body.workspaces).toEqual(["https://github.com/octo/hello.git#dev"]);
