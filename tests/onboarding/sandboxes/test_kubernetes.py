...
def test_build_job_manifest_github_token_rides_secret_ref_not_the_spec() -> None:
    ...
    manifest = build_job_manifest(
        **_MANIFEST_KW,
        ...
        ssh_authorized_keys=("ssh-ed25519 AAAAKEY octo@cat",),
    )
    ...
    for container in (
        _pod_spec(manifest)["containers"][0],
        _pod_spec(manifest)["initContainers"][0],
    ):
...
def test_build_job_manifest_no_github_token_has_no_credential_env() -> None:
    """Without a user token, no GIT_TOKEN/GH_TOKEN credential env is added."""
    manifest = build_job_manifest(**_MANIFEST_KW)
    host_env = {e["name"] for e in _pod_spec(manifest)["containers"][0]["env"]}
...
def test_resolve_pod_resources_defaults_leave_ephemeral_storage_unset() -> None:
    ...
    # Fork policy: memory is guaranteed (request == limit) but CPU has a floor
    # and no ceiling (burstable), so there is no ``limits.cpu``.
    assert resources == {
        "requests": {"cpu": k8s._SANDBOX_CPU_REQUEST, "memory": k8s._SANDBOX_MEMORY_REQUEST},
        "limits": {"memory": k8s._SANDBOX_MEMORY_LIMIT},
    }
...
def test_resolve_pod_resources_forwards_ephemeral_storage_in_both_tiers() -> None:
    """A configured ephemeral-storage request / limit reaches the Pod-level resources."""
    manifest = build_job_manifest(
        **_MANIFEST_KW,
        resources={
    ...
    res = _pod_spec(manifest)["resources"]
    assert res["requests"] == {
...
def test_build_job_manifest_config_map_mounts_land_on_host_container_only() -> None:
...
def test_build_job_manifest_budgets_resources_at_pod_level() -> None:
    """Resources live on the Pod (shared by host + sidecar), not per-container.
