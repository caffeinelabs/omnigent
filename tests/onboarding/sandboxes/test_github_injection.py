...
so a connected user's ``gh`` / git auth lands in the sandbox. See
...
def test_setup_commands_empty_without_identity() -> None:
    assert github_sandbox_setup_commands("/root", github_token=None, github_login=None) == []


def test_setup_commands_write_gh_hosts_yml_and_git_credentials() -> None:
    cmds = github_sandbox_setup_commands("/root", github_token="ghu_tok", github_login="octocat")
    joined = "\n".join(cmds)
...
def test_setup_commands_no_gh_config_without_identity() -> None:
    ...
    cmds = github_sandbox_setup_commands("/root", github_token=None, github_login=None)
    joined = "\n".join(cmds)
...
def test_start_host_injects_gh_and_env() -> None:
    ...
    # gh config + git credential were written via run().
...
def test_start_host_no_identity_is_unchanged() -> None:
    """Without a token, no gh commands and no credential env leak in."""
    launcher = _RecordingLauncher()
    launcher.start_host(
        "sb-1",
        token="tok",
