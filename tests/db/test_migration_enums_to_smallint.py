"""Tests for the enums-varchar→SMALLINT migration (``o1a2b3c4d5e6``).
...
def _seed_legacy_rows(engine: Engine) -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            sa.text(
                "INSERT INTO policies (id, name, created_at, type, handler, enabled) VALUES "
                "('p1', 'n1', 1, 'python', 'h', 1), ('p2', 'n2', 1, 'url', 'http', 1)"
            )
        )
        conn.execute(
            sa.text(
