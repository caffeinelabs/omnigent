...
async def test_auto_create_pi_terminal_threads_spec_model_into_models_json(
    ...
    # threaded override first, then the family's curated shortlist
    # (``claude-sonnet-4-6``); the override remains selected via ``--model``.
    ...
    # The family's curated shortlist model registers first (bare id), then the
    # selected override, appended with vision input on the anthropic surface.
