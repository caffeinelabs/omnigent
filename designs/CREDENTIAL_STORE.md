...
implementation and the only one shipped now. A `build_secret_cipher(config)`
factory is the single seam where a deployment selects a backend. Planned
adapters (not built until a deployment needs them, so the interface is designed
against a real second backend rather than speculatively):
